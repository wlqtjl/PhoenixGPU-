...
//
// Domain types are defined in pkg/types and re-exported here as aliases
// so handlers and tests continue to compile without import changes.
//
...
// ── Type aliases — re-export from pkg/types so handlers don't need a new import ──

type ClusterSummary = types.ClusterSummary
type GPUNode = types.GPUNode
type PhoenixJob = types.PhoenixJob
type SnapshotSummary = types.SnapshotSummary
type DeptBilling = types.DeptBilling
type BillingRecord = types.BillingRecord
type Alert = types.Alert
type TimeSeriesPoint = types.TimeSeriesPoint
type K8sClientInterface = types.K8sClientInterface

// ErrNotFound is re-exported from pkg/types.
